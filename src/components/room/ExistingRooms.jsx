...
const ExistingRooms = () => {
    ...
    useEffect(()=>{
        ...
        setCurrentPage(1)
    },[rooms,selectedRoomType]) //if rooms or selectedRoomType is changed, re-execute this function
    ...
    const calculateTotalPages = (filteredRooms, roomsPerPage, rooms) => {
      const totalRooms = filteredRooms.length > 0 ? filteredRooms.length : rooms.length
        return Math.ceil(totalRooms / roomsPerPage) //向上取整
    }

    const indexOfLastRoom = currentPage * roomsPerPage
    const indexOfFirstRoom = indexOfLastRoom - roomsPerPage
    //一页要展示的rooms
    const currentRooms = filteredRooms.slice(indexOfFirstRoom,indexOfLastRoom) //slice 左闭右开
